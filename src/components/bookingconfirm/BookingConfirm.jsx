...
const BookingConfirm = () => {
  ...
  const bookingId = location.state.json._id;//get the _id variable on the location object
  const { user, server } = useAuthContext();
  const { dispatch } = useBookingContext();
  const navigate = useNavigate();

  //delete a booking from the database by using the fetch method
  const removeBooking = async (id) => {
    ...
    // log the error to the browser console when it occurs
    if (!response.ok) {
